jQuery(function($) {
  ...
  var actionButtons = [{
    ...
    events: {
      click: function() {
          var formDataStr = JSON.parse(window.sessionStorage.getItem('formData'))
          var formData = JSON.parse(formDataStr)

          var converted = formData.map(function(item) {
              if(item.type === 'text') {
                  return {
                      key: item.name,
                      type: 'edit_text',
                      ...
                      v_required: {
                          ...
                          err: item.required ? 'please fullfil current value for '+item.name : ''
                      }
                  }
                  ...
                  return {
                      ...
                      values : item.values.flatMap(x => x.value)
                  }
              } else {
                  return {}
              }
          ...
          $.ajax({
              type: 'post',
              url: 'http://localhost:9527/forms/59c5ff5bc19efea6cc9a3726',
              contentType: "application/json; charset=utf-8",
              dataType   : "json",
              data: JSON.stringify(androidForm),
              success: function() {
                alert('saved');
              },
